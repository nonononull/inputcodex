...
`inputcodex` 是一个面向 Codex 本地增强与管理场景的原生桌面项目。项目以性能、稳定性、可诊断性和
跨平台一致性为优先目标，使用 Rust 与 Iced 重新实现有效能力，不沿用上游的 Tauri/React 架构。

> [!IMPORTANT]
> 项目仍处于开发阶段，尚未发布面向普通用户的正式安装包。当前成果主要是领域、应用和平台层能力，
> 桌面展示层仍是最小骨架，不能代表完整管理界面已经完成。
...
### 设置、诊断、本地会话与 Markdown

设置文件只返回顶层 JSON object 数量，合法空对象仍为 `Ready`；诊断日志仅观察固定文件尾部
最多 `256 KiB`，返回合法/损坏记录计数和截断事实。本地会话目录从受控 SQLite 来源返回会话 ID、
有界标题、归档状态、更新时间、分页和来源覆盖；不执行删除、备份或恢复。选定既有会话后，可从
严格只读 SQLite 与受控 rollout 在内存中生成固定 UTC/LF 的 Markdown，只包含用户/助手文本和
不可联网的图片占位；建议文件名不含完整会话 ID，生成结果不会自动保存为文件。Watcher 偏好状态
只检查固定 `watcher.disabled` 的元数据：标记缺失表示默认启用偏好，普通文件表示显式禁用偏好；
不读取内容、不返回实际路径，也不表示 Watcher 已安装或正在运行。偏好变更只接受 typed 的
`expected / desired`，在应用自有状态根原子创建或条件删除该固定空标记，并始终返回包含提交点与最终观察的收据；
它不安装、启动、停止或探测 Watcher。

除上述固定偏好标记变更外，这些能力不写入产品文件；全部能力均不打开 UI、不联网、不启动后台线程，
也不执行更新或环境清理。
...
正式改动使用 `Issue → 分支 → 验证证据 → PR → Review/CI → Squash Merge`。禁止直接向 `main` 写功能、force push、删除 `main`，所有 Review 对话都必须在确定根因并完成验证后解决。
