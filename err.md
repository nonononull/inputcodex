...
- 复审纠正：首版只校验 owner/blocker 格式，未绑定 22 个 feature 的批准值，`model-catalog` 又漏记 credential profile；#145 完成或 hard-stop 后也没有进入关闭任务并重开 #140 的复合 terminal。两个 PowerShell 生产边界还会经函数输出管道展开单元素数组，且 `-is [pscustomobject]` 不能区分根 object 与 `Object[]`，使 policy、PR evidence、Review/scope/workflow/Artifact 与 repository setting 的数组形状可能被投影成标量。修正为 feature 级精确 oracle、补齐四条 model-catalog owner kind、接入 #145 terminal，并保留属性原始类型、对根对象做精确运行时类型检查、对字符串执行类型加大小写精确比较；真实 JSON 数组反例覆盖两层生产边界。PR delivery 同步补自治 marker，不用 GitHub 元数据掩盖状态机缺口。
- 验证：首版 Control Plane RED 为 `75 pass / 7 fail`，首版 GREEN 为 `84/84`；复审纠正后完整合同为 `CI_CONTRACT_GREEN passed=88`，policy 校验、生产 helper、普通 refactor、upstream-sync、merge 与 post-merge 全部通过。Parity RED 先以缺失 `AdmissionOwnerMismatch` / `AdmissionBlockerMismatch` 失败；最终 admission `6/6`、目录 `33/33`、all-targets、Clippy 与 rustfmt 通过，动态计数保持 `22 feature / 83 source = 16/70 write + 2/5 process + 4/8 network`。
