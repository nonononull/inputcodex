...
- 处理：新增纯 `Get-AutonomousScopeProjection`，在 helper 内对过滤排序后的结果重新执行 `@(...)` 归一化，并统一返回数组、Int64 count 与带前缀 scope hash；live 只消费该投影。
- 验证：生产 helper 合同先因函数缺失稳定 RED；GREEN 后空输入返回 count `0`、空数组和 LF 空载荷哈希 `sha256:01ba4719c80b6fe911b091a7c05124b64eeece964e09c058ef8f9805daca546b`，完整 CI 合同为 67/67，真实 live 恢复为 active-worktree-execution / resume-worktree。
...
- 验证：生产 helper 合同先因函数缺失稳定 RED；GREEN 后 exact closed/open 两条路径通过，stale main、错误 Head、非 owner PR 与重复候选均保持零恢复，完整 CI 合同为 68/68。PR #113 合并后还必须以 `Closes #113` 完成无需人工 reopen 的真实 live 验收。
