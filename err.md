...
### 2026-08-01：`Path::exists` 折叠元数据错误且目录归属掩盖只读入口

- 环境：Issue `#127` / `#128` 审计上游 `load_watcher_state -> watcher_payload -> default_watcher_disabled_flag` 调用链，并从完整 Watcher 管理中拆分固定偏好标记观察。
...
- 复审纠正：首版只校验 owner/blocker 格式，未绑定 22 个 feature 的批准值，`model-catalog` 又漏记 credential profile；#145 完成或 hard-stop 后也没有进入关闭任务并重开 #140 的复合 terminal。两个 PowerShell 生产边界还会经函数输出管道展开单元素数组，且 `-is [pscustomobject]` 不能区分根 object 与 `Object[]`，使 policy、PR evidence、Review/scope/workflow/Artifact 与 repository setting 的数组形状可能被投影成标量。首轮修正为 feature 级精确 oracle、补齐四条 model-catalog owner kind、接入 #145 terminal，并保留属性原始类型、对根对象做精确运行时类型检查、对字符串执行类型加大小写精确比较；PR delivery 同步补自治 marker。后续复审又证明 `required_workflows[].name`、`workflow_runs[].name`、snapshot owner/base 身份仍在使用集合比较，且多个活动 Issue 会让 #145 hard-stop 丢失复合 terminal；第二轮将 Workflow run/job、Issue/PR owner 与 base ref 改为原始属性投影和精确字符串比较，并从全部可信活动 Issue 中精确识别唯一 #145。第三轮继续发现 Workflow 的 GitHub PR 关联 `head.sha/base.ref`、trusted issue number、merged PR base 与 post-merge review evidence 尚未对称验证；最终将 Workflow collector 全部字符串身份改为精确类型比较，为 issue number 增加 Int64 hard-stop，并让 post-merge 明确绑定标量 `main` 和 evidence/attestation 双 review 门，不用 GitHub 元数据或单 Issue 假设掩盖状态机缺口。
- 验证：首版 Control Plane RED 为 `75 pass / 7 fail`，首版 GREEN 为 `84/84`；首轮复审纠正后为 `88/88`。第二轮新增 policy/PR/main Workflow 名称、四个 owner/base 身份和多 Issue terminal 反例，旧生产面稳定得到 `5` 个合同失败，修正后为 `90/90`。第三轮为 Workflow PR 关联、issue number、merged PR base 与 post-merge review evidence 建立四个独立 RED，最终完整合同为 `CI_CONTRACT_GREEN passed=92`，policy 校验、生产 helper、普通 refactor、upstream-sync、merge 与 post-merge 全部通过。Parity RED 先以缺失 `AdmissionOwnerMismatch` / `AdmissionBlockerMismatch` 失败；最终 admission `6/6`、目录 `33/33`、all-targets、Clippy 与 rustfmt 通过，动态计数保持 `22 feature / 83 source = 16/70 write + 2/5 process + 4/8 network`。
