...
Invoke-ContractTest -Name '性能基线 Workflow 固定治理与测量合同' -Body {
    ...
    Assert-True -Condition ($workflow.Contains('elseif ($presentCount -eq 0) {')) -Message '自动事件在证据全缺失时必须进入测量路径'
    Assert-True -Condition ($workflow.Contains('elseif ($presentCount -eq $evidencePaths.Count) {')) -Message '自动事件在证据完整时必须进入 Evidence 路径'
    Assert-True -Condition ($workflow.Contains('性能证据必须全部缺失或全部存在')) -Message '自动事件证据部分缺失时必须明确失败'
