...
function New-ValidAutonomousRefactorPolicy {
    [pscustomobject][ordered]@{
        ...
        side_effect_admission_matrix = [pscustomobject][ordered]@{
            ...
            decision_id = 'gate5-governance-recovery-v1/batch-2'
            ...
            standing_authorization_ref = 'https://github.com/nonononull/inputcodex/issues/111'
            baseline_commit = 'c26e97ee534b74ebe1252346477640dc196f89b9'
...
Invoke-ContractTest -Name '副作用准入任务硬停止进入 owner terminal transition' -Body {
    $snapshot = Copy-AutonomousStateSnapshot (New-ValidAutonomousStateSnapshot)
    $snapshot.branch = 'codex/issue-149-gate-5-side-effect-admission-matrix-successor'
    $snapshot.active_writer_count = 2L
    $snapshot.active_issues = @([pscustomobject]@{
        number = 149L
        url = 'https://github.com/nonononull/inputcodex/issues/149'
        author_login = 'nonononull'
    })
    ...
    $multipleIssues.branch = 'codex/issue-149-gate-5-side-effect-admission-matrix-successor'
...
Invoke-ContractTest -Name '副作用准入任务合并后进入 owner terminal transition' -Body {
    $scopeHash = 'sha256:7269cdd0eb2726d967bca4f1f183a2c4f7082bf51312cb19ba31432bae0809c5'
    $snapshot = New-PostMergeAutonomousStateSnapshot
    $snapshot.branch = 'codex/issue-149-gate-5-side-effect-admission-matrix-successor'
    $snapshot.actual_scope_count = 20L
    $snapshot.actual_scope_hash = $scopeHash
    $snapshot.active_issues[0].number = 149L
    $snapshot.active_issues[0].url = 'https://github.com/nonononull/inputcodex/issues/149'
    ...
    $snapshot.active_issues[0].planning_evidence.ref = 'https://github.com/nonononull/inputcodex/issues/149#issuecomment-5176946126'
    $snapshot.merged_prs[0].number = 150L
    $snapshot.merged_prs[0].url = 'https://github.com/nonononull/inputcodex/pull/150'
    $snapshot.merged_prs[0].evidence.tracking_issue_ref = 'https://github.com/nonononull/inputcodex/issues/149'
