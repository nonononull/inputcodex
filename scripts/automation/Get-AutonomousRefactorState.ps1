...
function Get-SideEffectAdmissionMatrixProjection {
    ...
    $stringsValid =
        ...
        $decisionId -is [string] -and
        $decisionId -ceq 'gate5-side-effect-admission-matrix-successor-v3' -and
        $ownerDecisionRef -is [string] -and
        $ownerDecisionRef -ceq 'https://github.com/nonononull/inputcodex/issues/140#issuecomment-5191294948' -and
        $trackingIssueRef -is [string] -and
        $trackingIssueRef -ceq 'https://github.com/nonononull/inputcodex/issues/163' -and
        $standingAuthorizationRef -is [string] -and
        $standingAuthorizationRef -ceq 'https://github.com/nonononull/inputcodex/issues/111' -and
        $baselineCommit -is [string] -and
        $baselineCommit -ceq '5a7465252b56f7e90673e72d3e02881ac9238141' -and
...
$sideEffectAdmissionIssues = @($activeIssues | Where-Object {
    $issueUrlProperty = $_.PSObject.Properties['url']
    $null -ne $issueUrlProperty -and
    $issueUrlProperty.Value -is [string] -and
    $issueUrlProperty.Value -ceq $sideEffectAdmissionTrackingIssueRef
})
...
if ((Get-PropertyValue $snapshot 'github_available') -eq $true -and
    (Get-PropertyValue $snapshot 'paseo_available') -eq $true -and
    $activeIssues.Count -eq 0 -and
    $activePrs.Count -eq 0 -and
    $linkedMergedPrs.Count -eq 0) {
    $reasonCodes.Add('NO_AUTHORIZED_CANDIDATE') | Out-Null
}
