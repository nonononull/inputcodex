...
function Get-GitHubPostMergeEvidence {
    ...
    foreach ($parent in $parentsProperty.Value) {
        ...
        if ($null -eq $shaProperty -or
            ...
            $shaProperty.Value -cnotmatch '\A[0-9a-f]{40}\z') {
            throw 'gh post-merge parent schema invalid'
        }
...
function Get-AutonomousPostMergeGateEvaluation {
    ...
    if ($null -eq $postMerge -or
        ...
        $parentCount -ne 1 -or
        $expectedBase -isnot [string] -or
        $expectedBase -cnotmatch '\A[0-9a-f]{40}\z' -or
        $parentOid -isnot [string] -or
        $parentOid -cnotmatch '\A[0-9a-f]{40}\z' -or
...
if ((Get-PropertyValue $snapshot 'schema_version') -cne 'inputcodex.autonomous-refactor-state-snapshot.v1' -or
    ...
    $expectedBaseValue -cnotmatch '\A[0-9a-f]{40}\z' -or
