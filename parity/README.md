...
- 当前共有 46 个 feature，其中 11 个为 `exception-pending`。
...
- Issue `#132` 已将 `feature.remote-install.zed-remote-project-observation` 固定为 Zed 远程项目只读观察切片：只接管 `tauri-command:list_zed_remote_projects`，有界读取固定全局状态与严格只读 SQLite，只返回完整 SHA-256 稳定假名、来源、`SelectedHostHint / NotObserved` 和来源覆盖；稳定假名不是匿名标识，原 open/forget/core 管理能力继续 `unassessed`。
...
- 当前五域共保存 `46` 份行为合同，并为 `13` 个需要结构数据的 feature 保存 `13` 个 fixture manifest；其余场景以合同中的 `fixture_policy: none` 说明无需 fixture。
