...
- 当前共有 46 个 feature：13 个 `implemented`、22 个 `unassessed`、11 个 `exception-pending`。
...
- `admission/side-effect-admission-matrix.yml` 一对一覆盖当前 `83` 个 `unassessed` source；它是治理输入，不是产品运行数据。
- 桶优先级固定为 write、process、network，结果为 write `16 feature / 70 source`、process `2 / 5`、network `4 / 8`。
- 每项固定 typed owner 的 `missing / partial` 状态、所需 owner kind 与既有 blocker ref；重复、遗漏、未知、feature 归属或桶漂移均由仓库验证拒绝。
- 全部条目固定为 `admission: blocked` 与 `implementation_authorized: false`。矩阵不批准实现，不改变现有 feature/source disposition，也不解锁 Gate 5 或 Gate 6。
- 后续 owner 决策若授权任何 source，必须使用独立 Issue/PR 修改机器合同；不得从矩阵顺序、相邻只读切片或 partial owner 推导 standing authorization。
...
- Issue `#128` 已将 `feature.foundation-platform.watcher-preference-observation` 固定为 Watcher 偏好状态只读观察切片：只接管 `tauri-command:load_watcher_state`，仅检查 `inputcodex_state_root/watcher.disabled` 元数据；缺失表示 `EnabledByDefault`，普通文件表示 `ExplicitlyDisabled`，不读取内容、不写入、不控制进程，原 Watcher 管理总功能继续 `unassessed`。
- Issue `#143` 已将 `feature.foundation-platform.watcher-preference-mutation` 固定为 cooperative-same-user 威胁模型下的单文件变更切片：只接管 `tauri-command:disable_watcher` 与 `tauri-command:enable_watcher`，按 expected/desired 原子创建或条件删除固定 `watcher.disabled`，提交后始终交付 receipt；完整 Watcher 的 core、安装与卸载仍保持 `unassessed`。
...
- 当前五域共保存 `46` 份行为合同，并为 `12` 个需要结构数据的 feature 保存 `12` 个 fixture manifest；其余场景以合同中的 `fixture_policy: none` 说明无需 fixture。
