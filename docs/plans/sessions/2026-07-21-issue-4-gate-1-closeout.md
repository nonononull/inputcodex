  ...
  protected_feature_replay:
    ...
    known_good_features:
      - feature: Issue #2 / PR #3 已合并治理基线
        ...
        post_change_replay_ref: local:2026-07-21:PR3_MERGE_EVIDENCE_PREWRITE=passed
...
- 当前分支正常推送并创建包含 `Closes #4` 的开放 PR；回写真实 PR URL后再次验证和推送。
- PR 保持 OPEN、非 Draft、无未解决 Review 对话；本任务不自动合并。
