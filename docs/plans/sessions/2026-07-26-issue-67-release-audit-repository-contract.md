...
allowed_operations: exact-six-path-equivalent-migration, local-red-green, temporary-pr66-overlay, normal-commit, normal-push, replacement-non-draft-pr, review-ci, mark-pr68-superseded-after-green
mutation_intent: control-plane
executor_enforcement: exact-six-path-set, no-pr66-mutation, no-pr68-mutation, fresh-verification, normal-push-only, squash-merge-only
...
delivery_evidence:
  ...
  merge_ref: pending
  predecessor_pr_ref: https://github.com/nonononull/inputcodex/pull/68
  predecessor_status: keep-open-until-replacement-green
master_plan:
...
- Decision：采用方案 A，只解除仓库实例测试对活动快照状态的错误固定，保留目录证据和专项状态机合同；因 PR `#68` 与新 `main` 的 `err.md` 发生真实冲突，基于当前 `main` 建立六路径等价替代 PR。
...
brainstorming:
  ...
  user_decision: 项目所有者批准基于当前 main 建立六路径等价替代分支与新 PR，保持相同功能 diff 和 scope_hash；禁止修改或 force push PR #68；替代 PR 全绿后再标记旧 PR superseded，最终 Squash Merge 保留单独授权门。
  ...
  approved_decision_evidence:
    ...
    - https://github.com/nonononull/inputcodex/issues/67#issuecomment-5084412075
  options_considered:
