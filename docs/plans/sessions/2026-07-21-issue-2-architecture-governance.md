...
merge_policy: 只创建待审 PR；项目所有者审阅通过且检查成功后合并
closeout_ref: pending:docs/reports/issue-2-architecture-governance-closeout.md
  ...
  protected_feature_replay:
    ...
    known_good_features:
      - feature: Gate 0 仓库筹备基线
        ...
        post_change_replay_ref: pending:PR-checks
...
pr_ref: pending:create-pr
ci_ref: pending:pr-checks-or-not-configured
...
merge_policy: PR 正文必须包含 Closes #2；项目所有者批准前不合并。
```
...
- Fresh 验证通过，变更提交到 `docs/issue-2-architecture-governance` 并推送。
- 创建包含 `Closes #2` 的 PR，等待项目所有者 Review；本会话不自动合并。
