...
- 项目所有者已批准将 `AGENTS.md` 纳入 Issue `#28` 的状态回写范围；该批准只允许更正当前 Gate 状态，不扩大产品、性能、上游或 CI 写入面，引用为 `user-message:approve-issue-28-scope-amendment-agents-md-2026-07-22`。

## 三、范围与文件职责

本任务只允许以下八条路径，路径升序、LF 分隔并保留末尾 LF 后得到本计划顶部的 `scope_hash`：

1. `AGENTS.md`：将当前 Gate 状态从已完成的 Issue `#26` / PR `#27` 更正为当前 Issue `#28` / PR `#29` Closeout，并维持性能基线锁定条件。
2. `README.md`：将当前阶段描述从“PR 待创建”更新为已合并的 Gate 4 功能目录状态，并明确下一步是独立 Closeout 后的性能基线 Issue。
3. `docs/plans/2026-07-22-issue-28-gate-4-feature-catalog-closeout.md`：记录本实施计划、范围、步骤与停止条件。
4. `docs/plans/PROJECT-MASTER-PLAN.md`：回写活动任务、Gate 4 清单和后续性能基线前置条件。
5. `docs/plans/sessions/2026-07-22-issue-28-gate-4-feature-catalog-closeout.md`：记录会话决策、范围、AGOS 绕过原因和执行检查点。
6. `docs/reports/issue-26-gate-4-feature-catalog.md`：将来源 PR、CI、Review、Squash 合并与分支清理字段回写为真实证据。
7. `docs/reports/issue-28-gate-4-feature-catalog-closeout.md`：形成 Closeout 证据报告并记录本 PR 的动态收口状态。
8. `docs/workflows/2026-07-22-issue-28-gate-4-feature-catalog-closeout-runtime.md`：定义逐批执行、停机与验证流程。

路径升序、LF 分隔并保留末尾 LF 的 SHA-256 为 `a6559a9538dc16908d76c6a0360c1b7c7cb3323158764d244ca891f49d587e1a`。
...
- [ ] 在实施计划、Session Plan、Runtime Workflow、Closeout 报告与 `AGENTS.md` 中统一记录 Issue `#28`、来源 Issue/PR、原始批准与范围扩展批准、基线、八路径范围、`scope_hash`、`allowed_operations`、`mutation_intent` 与 `executor_enforcement`。
- [ ] 记录 AGOS `-ReportOnly` 返回 `needs-input` 与 `unregistered`；按项目规则绕过，不修改 AGOS、Registry、Workflow 或 Vault。
- [ ] 运行本计划的范围自检：以基线差异路径和未跟踪文件的并集确认工作树只包含八条允许路径，且范围哈希等于 `sha256:a6559a9538dc16908d76c6a0360c1b7c7cb3323158764d244ca891f49d587e1a`。
...
- [ ] 使用 `git diff --name-only <baseline>` 与 `git ls-files --others --exclude-standard` 的并集检查工作树路径集合，与八路径允许集合完全一致。
...
- [ ] 范围扩展形成新的 PR Head 后，旧 Head 的 CI 只保留为历史证据；必须等待新 Head 的 GitHub-hosted CI 成功才可进入合并前 Fresh 验证。
