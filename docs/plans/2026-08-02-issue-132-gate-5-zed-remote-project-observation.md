...
- [x] RED：固定 ID 格式、三种 origin、两种选择提示、来源覆盖、项目/来源计数和脱敏 Debug。
- [x] GREEN：实现纯领域值；禁止原始身份材料、label、路径、URL 或时间戳字段。
- [x] 运行 Domain 专项、全目标 Clippy 与 rustfmt，建立 domain checkpoint `d5f7b4f`。

## Task 3：Application RED -> GREEN

- [x] RED：固定零字段 Request、Cancellation、Port、Ready/Partial/Empty/Failed 与取消后迟到 `Stale`。
- [x] GREEN：复用 `LoadCompletion` 与 `LoadCoordinator`，不新增通用 Partial 状态。
- [x] 运行 Application 专项、全目标 Clippy 与 rustfmt，建立 application checkpoint `726d30b`。

## Task 4：Platform RED -> GREEN

- [x] RED：SHA-256 向量、None/22 区分、碰撞、敏感值零输出、JSON/SQLite 资源矩阵、NOFOLLOW、超时取消和 WAL 只读证据。
- [x] GREEN：实现固定路径、有界同句柄 JSON、严格只读 SQLite、确定性匹配/排序和完整性分类。
- [x] 仅添加 `sha2 0.10.9` 依赖闭包；Platform 专项 `12/12`、Clippy 与 rustfmt 通过，建立 checkpoint `2873f94`。

## Task 5：Parity RED -> GREEN

- [x] RED：要求新 feature/contract/fixture 与 `[filesystem-read, database-read]`；旧 Zed 总功能不得再包含 list。
- [x] GREEN：只移动 `list_zed_remote_projects`；旧 core/open/forget 与旧完整 fixture 保持。
- [x] 计数固定为 `135/46/46/13/11/3/0`，建立 parity checkpoint `bda6a81`。

## Task 6：文档、本地门禁与远端交付

- [x] 更新稳定 README、术语、主计划、报告和构建命令，不在 README 写动态 Head/CI。
- [x] 运行 `build.md` Issue #132 全部轻量门禁，实际路径精确为 28。
