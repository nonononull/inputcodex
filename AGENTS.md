...
- Gate 3 七成员纯 Rust Workspace、首版三平台 CI、失败语义和冷构建最低基线已通过 Issue `#19` / PR `#21` 进入 `main`；该 Gate 当时只交付最小应用骨架，尚未迁移上游业务功能，后续能力迁移由 Gate 5 独立 Issue/PR 完成。
...
- Issue `#95` / PR `#96` 已完成第七个 Gate 5 诊断日志只读结构观察切片；PR `#96` 以单父 Squash 提交 `b7c4174671caba806162a42e82b7bc0b20f73bf5` 进入 `main`，Issue `#95` 已按 `COMPLETED` 关闭。
...
- Issue `#115` / PR `#122` 已完成 `v1.2.44` 功能目录重审；Final Head `c1b1a173f8a0b1fe986d7238219a75214aa01a86` 以单父 Squash 提交 `29f5317d66b9f16cf64797420bf2fd7e2aec45f7` 进入 `main`，目录计数为 `135/44/44/13/10/3/0`，Issue 已按 `COMPLETED` 关闭，合并后 CI 七 Job、Performance Baseline 四 Job全绿且 Artifact 均为 `0`。
- Issue `#123` 将错误登记的 `feature.session-data.token-usage-history` 重分类为 `feature.session-data.local-storage-model-suffix-sanitization`：该入口真实依赖 CDP、JavaScript、Electron Local Storage 写入和诊断日志写入，必须保持 `exception-pending`；任务固定十六路径与 `sha256:aa27e2551cfa743248ef7a2ab53fad5f1a1954b369ae40bf3485ada2099f7bdc`，真正的 rollout Token 历史必须另建独立 Discovery。
...
- Issue `#137` 已确认当前正式 Release 与目录基线下的只读候选前沿饱和，但 `gate_5_product_complete=false`、`gate_6_unlocked=false`；Issue `#138` 只允许在十二路径与 `sha256:aac82afc513192fa9adbe1ed6b85f81c56fa3bd8cd3ea20e718d1e31c794f417` 内增加候选耗尽自治终态，不得借机降低产品安全门或进入正式发布。
