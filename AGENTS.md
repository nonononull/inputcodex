...
- Issue `#75` 是首个 Gate 5 产品迁移切片；批准范围为三十路径与 `sha256:ae5e0f5143355feee9b280da7c44fdd5cdf759ec2ae71fc69167040bf302cb37`。当前分支已完成领域、应用、Windows/macOS 平台适配器、Parity RED/GREEN 与最终本地轻量门禁：四 crate 测试/Clippy、CI 合同 `35/35`、`release_audit=current`、仓库政策 `0` 违规、范围 `30/30` 和隐私检查均通过；未加入 UI、写入、网络、缓存、广告、远程推荐或 `unsafe`。非 Draft PR、Hosted Review/CI 与最终 Squash Merge 仍待后续门禁，最终合并必须单独授权。
- Gate 5 已解锁，但只能按独立功能 Issue/PR 串行推进；Issue `#75` 未完成前不得夹带第二个产品 feature、UI、预算、Release、`upstream/`、Ruleset 或 AGOS 改动。
