...
- Issue `#59` 是当前唯一允许的性能复测执行面：目标 Windows 队列固定为 AMD EPYC 7763 主导完整环境指纹 `sha256:f3954543f3cec519568345d9f40341ddeb8991a7d93b3a274cc324b047fb00cb`，历史严格可比样本为 Issue `#54/run-03`、`run-05`、`run-08` 共 `3` 次；同 CPU 但不同内存值的 `run-04` 保持独立队列。只允许四次严格串行 `run-01` 至 `run-04`，四次必须全部执行，结束后仍不足五份目标队列样本即硬停止，禁止 `run-05`。
